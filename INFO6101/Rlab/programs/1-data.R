...
## first param?te? is the data frame, the remaining are column names
...
## (4) "extract di?tinct (unique) rows"
select(df, S.F.Ratio)
...
mutate(df, cost=Personal+Book?)

dfx <- select(df,Personal,Books)
dfx <- mutate(dfx, cos?=Personal+Books)
dfx
...
group_by(df, Private) %>% summarise(max = max(Outstate), min = min(Outstate))
group_by(college, Private) %>% summarise(max = max(Outstate), min = min(Outstate))
...
?# http://www.rstudio.com/wp-content/uploads/2015/02/data-wr?ngling-cheatsheet.pdf
